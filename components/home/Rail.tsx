...
export default function Rail({ title, cards }: { title: string; cards: RailCard[] }) {
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      <FocusableSection as="div" className="flex gap-5 overflow-x-auto pb-2">
        {cards.map((card) => (
          <FocusableLink
            key={card.key}
            href={card.href}
            className="group flex w-48 shrink-0 flex-col gap-2 transition hover:-translate-y-1"
          >
            <div
            ...
            <div>
              <p className="truncate text-base font-medium">{card.title}</p>
              {card.subtitle && <p className="truncate text-sm text-muted">{card.subtitle}</p>}
            </div>
          </FocusableLink>
        ))}
      </FocusableSection>
